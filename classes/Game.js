...
const { FIELD_SIZE, MARKERS } = require('../constants');
...
class Game {
  constructor(player1, player2, room = null, swapSides = true) {
    this.id = nanoid();
    this.field = getEmptyField(FIELD_SIZE);
    this.room = room;
  ...
  getWinner() {
    ...
    if (winnerMarker.isEqual(this.player1.marker)) {
      return this.player1.id;
    }
    if (winnerMarker.isEqual(this.player2.marker)) {
